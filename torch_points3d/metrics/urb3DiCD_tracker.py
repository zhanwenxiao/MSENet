...
class Urb3DiCDTracker(iCDTracker):
    ...
    def track(self, model: model_interface.TrackerInterface, data=None, full_pc=False, conv_classes=None,**kwargs):
        ...
        inputs.change_target_pred = change_output
    ...
    def change_metrics(self, model, data_l, num_class_pred):
        for p in range(len(data_l)):
            ...
            if self._areas[area_sel] is None:
                ...
                self._areas[area_sel].prediction_count = torch.zeros(self._areas[area_sel].change_y_target.shape[0], dtype=torch.int)
                self._areas[area_sel].votes = torch.zeros((self._areas[area_sel].change_y_target.shape[0], num_class_pred), dtype=torch.float)
            ...
            # Set predictions
            self._areas[area_sel].votes[originids] += data_l[p].change_target_pred
        ...
        if self.full_pc:
            if num_class_cm is None:
                if conv_classes is None:
                    num_class_cm = self._change_classes
                else:
            ...
            for i, area in enumerate(self._areas):
                if area is not None:
                    ...
                    if self.full_res:
                        ...
                        _, area_orig_pos, _, _, _, _, gt = self._ds.clouds_loader(i, "vertex")
                        ...
                        pred = pred.numpy()
                        gt = area.change_y_target[has_prediction].numpy()
                        pos = pos.cpu()
