...
def assign_instances_for_scan(pred: dict):
    ...
    for uuid in pred_info:
        ...
        for (gt_num, gt_inst) in enumerate(gt2pred[label_name]):
            ...
            if intersection > 0:
                # inst_idx = np.argwhere(np.logical_and(gt_ids == gt_inst["instance_id"], pred_mask)).reshape(-1, )
                # if pred["pred_changes"] is not None:
                #     inst_pred_changes = pred["pred_changes"][inst_idx]
                # else:
                #     inst_pred_changes = None
                # inst_gt_changes = pred["gt_changes"][inst_idx]
                _inst_idx = np.argwhere(gt_ids == gt_inst["instance_id"]).reshape(-1,)
                if pred["pred_changes"] is not None:
                    inst_pred_change = pred["pred_changes"][_inst_idx]
                    _inst_pred_mask = pred_mask[_inst_idx]
                    inst_pred_changes = np.where(_inst_pred_mask == 1, inst_pred_change, 0)
                else:
                    inst_pred_changes = None
                inst_gt_changes = pred["gt_changes"][_inst_idx]
                gt_copy = gt_inst.copy()
                pred_copy = pred_instance.copy()
                gt_copy["intersection"] = intersection
                pred_copy["intersection"] = intersection
                pred_copy["pred_changes"] = inst_pred_changes
                pred_copy["gt_changes"] = inst_gt_changes
                pred_copy["time"] = pred["time"]
...
def evaluate_matches(matches):
    ...
    match_change_pair_pred0, match_change_pair_pred1 = [], []
    match_change_pair_gt0, match_change_pair_gt1 = [], []

    for di, (min_region_size, distance_thresh, distance_conf) in enumerate(
        zip(min_region_sizes, dist_threshes, dist_confs)
    ):
        match_change_pair_pred_min_size0, match_change_pair_pred_min_size1 = [], []
        match_change_pair_gt_min_size0, match_change_pair_gt_min_size1 = [], []
        for oi, overlap_th in enumerate(overlaps):
            match_change_pair_pred_oi0, match_change_pair_pred_oi1 = [], []
            match_change_pair_gt_oi0, match_change_pair_gt_oi1 = [], []
            pred_visited = {}
            for m in matches:
            ...
            for li, label_name in enumerate(CLASS_LABELS):
                match_change_pair_pred_class0, match_change_pair_pred_class1 = [], []
                match_change_pair_gt_class0, match_change_pair_gt_class1 = [], []
                y_true = np.empty(0)
                y_score = np.empty(0)
                ...
                for m in matches:
                    ...
                    match_change_pair_pred_match0 = [None] * len(gt_instances)
                    match_change_pair_gt_match0 = [None] * len(gt_instances)
                    match_change_pair_pred_match1 = [None] * len(gt_instances)
                    match_change_pair_gt_match1 = [None] * len(gt_instances)
                    ...
                    for (gti, gt) in enumerate(gt_instances):
                        ...
                        for pred in gt["matched_pred"]:
                            ...
                            if overlap > overlap_th:
                                pred_changes = pred["pred_changes"]
                                gt_changes = pred["gt_changes"]
                                confidence = pred["confidence"]
                                time = pred["time"]
                                # if already have a prediction for this gt,
                                # the prediction with the lower score is automatically a false positive
                                if cur_match[gti]:
                                    if confidence > cur_score[gti]:
                                        if pred_changes is not None:
                                            if time == 0:
                                                match_change_pair_pred_match0[gti] = pred_changes
                                                match_change_pair_gt_match0[gti] = gt_changes
                                            else:
                                                match_change_pair_pred_match1[gti] = pred_changes
                                                match_change_pair_gt_match1[gti] = gt_changes
                    ...
                    match_change_pair_pred_match0 = [x for x in match_change_pair_pred_match0 if x is not None]
                    match_change_pair_gt_match0 = [x for x in match_change_pair_gt_match0 if x is not None]
                    match_change_pair_pred_match1 = [x for x in match_change_pair_pred_match1 if x is not None]
                    match_change_pair_gt_match1 = [x for x in match_change_pair_gt_match1 if x is not None]
                    ...
                    match_change_pair_pred_class0.append(match_change_pair_pred_match0)
                    match_change_pair_gt_class0.append(match_change_pair_gt_match0)
                    match_change_pair_pred_class1.append(match_change_pair_pred_match1)
                    match_change_pair_gt_class1.append(match_change_pair_gt_match1)
                ...
                match_change_pair_pred_oi0.append(match_change_pair_pred_class0)
                match_change_pair_gt_oi0.append(match_change_pair_gt_class0)
                match_change_pair_pred_oi1.append(match_change_pair_pred_class1)
                match_change_pair_gt_oi1.append(match_change_pair_gt_class1)
            match_change_pair_pred_min_size0.append(match_change_pair_pred_oi0)
            match_change_pair_gt_min_size0.append(match_change_pair_gt_oi0)
            match_change_pair_pred_min_size1.append(match_change_pair_pred_oi1)
            match_change_pair_gt_min_size1.append(match_change_pair_gt_oi1)
        match_change_pair_pred0.append(match_change_pair_pred_min_size0)
        match_change_pair_gt0.append(match_change_pair_gt_min_size0)
        match_change_pair_pred1.append(match_change_pair_pred_min_size1)
        match_change_pair_gt1.append(match_change_pair_gt_min_size1)
    return ap, match_change_pair_pred0, match_change_pair_gt0, match_change_pair_pred1, match_change_pair_gt1

# def get_points(pred_pt, gt_pt, region_id, metric_id):
#     _pred_pt = []
#     _gt_pt = []
#     for i in metric_id[0]:
#         region_pred = pred_pt[region_id][i]
#         region_gt = gt_pt[region_id][i]
#         f_pred_region_pt = [array for sublist in region_pred for m in sublist for array in m]
#         f_gt_region_pt = [array for sublist in region_gt for m in sublist for array in m]
#         if len(f_pred_region_pt) != 0 and len(f_pred_region_pt) != 0:
#             pred_region_pt = np.concatenate(f_pred_region_pt)
#             gt_region_pt = np.concatenate(f_gt_region_pt)
#             _pred_pt.append(pred_region_pt)
#             _gt_pt.append(gt_region_pt)
#     if len(_pred_pt) != 0 and len(_gt_pt) != 0:
#         _pred_pt = np.concatenate(_pred_pt)
#         _gt_pt = np.concatenate(_gt_pt)
#     return _pred_pt, _gt_pt

def get_points(pred_pt, gt_pt, region_id, oAllBut25, o50, o25):
    oAllBut25_pred_pt, o50_pred_pt, o25_pred_pt = [], [], []
    oAllBut25_gt_pt, o50_gt_pt, o25_gt_pt = [], [], []

    region_preds = pred_pt[region_id]
    region_gts = gt_pt[region_id]
    for i, (region_pred, region_gt) in enumerate(zip(region_preds, region_gts)):
        for obj_pred, obj_gt in zip(region_pred, region_gt):
            for match_pred, match_gt in zip(obj_pred, obj_gt):
                for pt_pred, pt_gt in zip(match_pred, match_gt):
                    if len(pt_pred) != 0 and len(pt_gt) != 0:
                        if i in oAllBut25[0]:
                            oAllBut25_pred_pt.append(pt_pred)
                            oAllBut25_gt_pt.append(pt_gt)
                        if i in o50[0]:
                            o50_pred_pt.append(pt_pred)
                            o50_gt_pt.append(pt_gt)
                        if i in o25[0]:
                            o25_pred_pt.append(pt_pred)
                            o25_gt_pt.append(pt_gt)
    # print("oAllBut25_pred_pt {} oAllBut25_gt_pt {}".format(oAllBut25_pred_pt, oAllBut25_gt_pt))
    # if len(oAllBut25_pred_pt) != 0 and len(oAllBut25_gt_pt) != 0:
    #     oAllBut25_pred_pt = np.concatenate(oAllBut25_pred_pt)
    #     oAllBut25_gt_pt = np.concatenate(oAllBut25_gt_pt)
    # print("o50 concatenate")
    # if len(o50_pred_pt) != 0 and len(o50_gt_pt) != 0:
    #     o50_pred_pt = np.concatenate(o50_pred_pt)
    #     o50_gt_pt = np.concatenate(o50_gt_pt)
    # print("o25 concatenate")
    # if len(o25_pred_pt) != 0 and len(o25_gt_pt) != 0:
    #     o25_pred_pt = np.concatenate(o25_pred_pt)
    #     o25_gt_pt = np.concatenate(o25_gt_pt)
    return oAllBut25_pred_pt, o50_pred_pt, o25_pred_pt, oAllBut25_gt_pt, o50_gt_pt, o25_gt_pt

def apchange_metric(match_c_pred, match_c_gt, d_inf, oAllBut25, o50, o25):
    change_pred_dict, change_gt_dict = {}, {}

    # ###### mAP-> Change mIoU ######
    # oAllBut25_pred_pt, oAllBut25_gt_pt = get_points(match_c_pred, match_c_gt, d_inf, oAllBut25)
    # ###### mAP-> Change mIoU ######
    #
    # ###### mAP50-> Change mIoU ######
    # o50_pred_pt, o50_gt_pt = get_points(match_c_pred, match_c_gt, d_inf, o50)
    # ###### mAP50-> Change mIoU ######
    #
    # ###### mAP25-> Change mIoU ######
    # o25_pred_pt, o25_gt_pt = get_points(match_c_pred, match_c_gt, d_inf, o25)
    # ###### mAP25-> Change mIoU ######
    oAllBut25_pred_pt, o50_pred_pt, o25_pred_pt, oAllBut25_gt_pt, o50_gt_pt, o25_gt_pt =  get_points(match_c_pred, match_c_gt, d_inf, oAllBut25, o50, o25)

    change_pred_dict["all_ap"] = oAllBut25_pred_pt
    change_pred_dict["all_ap_50%"] = o50_pred_pt
    change_pred_dict["all_ap_25%"] = o25_pred_pt

    change_gt_dict["all_ap"] = oAllBut25_gt_pt
    change_gt_dict["all_ap_50%"] = o50_gt_pt
    change_gt_dict["all_ap_25%"] = o25_gt_pt
    return change_pred_dict, change_gt_dict

def compute_averages(aps, match_c_pred0, match_c_gt0, match_c_pred1, match_c_gt1):
    d_inf = 0
    ...
    change_pred_dict0, change_gt_dict0 = apchange_metric(match_c_pred0, match_c_gt0, d_inf, oAllBut25, o50, o25)
    change_pred_dict1, change_gt_dict1 = apchange_metric(match_c_pred1, match_c_gt1, d_inf, oAllBut25, o50, o25)
    return avg_dict, change_pred_dict0, change_gt_dict0, change_pred_dict1, change_gt_dict1
...
class hkiCDTracker(iCDTracker):
    def __init__(self, dataset, stage="train", wandb_log=False, use_tensorboard: bool = False,
                 ignore_label: int = IGNORE_LABEL, full_pc: bool = False, full_res: bool = False):
        super(hkiCDTracker, self).__init__(dataset, stage, wandb_log, use_tensorboard, ignore_label)
    ...
    def get_inst_metric(self, change_pred_dict_0, change_pred_dict_1, change_gt_dict_0, change_gt_dict_1):
        length_pred = len(change_pred_dict_0["all_ap"]) + len(change_pred_dict_1["all_ap"])
        length_gt = len(change_gt_dict_0["all_ap"]) + len(change_gt_dict_1["all_ap"])
        length_pred50 = len(change_pred_dict_0["all_ap_50%"]) + len(change_pred_dict_1["all_ap_50%"])
        length_gt50 = len(change_gt_dict_0["all_ap_50%"]) + len(change_gt_dict_1["all_ap_50%"])
        length_pred25 = len(change_pred_dict_0["all_ap_25%"]) + len(change_pred_dict_1["all_ap_25%"])
        length_gt25 = len(change_gt_dict_0["all_ap_25%"]) + len(change_gt_dict_1["all_ap_25%"])
        if length_pred != 0 and length_gt != 0:
            for c_gt_0, c_pred_0, c_gt_1, c_pred_1 in zip(change_gt_dict_0["all_ap"], change_pred_dict_0["all_ap"], change_gt_dict_1["all_ap"], change_pred_dict_1["all_ap"]):
                self.apiou_confusion_matrix.count_predicted_batch_list(c_gt_0, c_pred_0, c_gt_1, c_pred_1)
            ap_change_miou = 100 * self.apiou_confusion_matrix.get_average_intersection_union()
        else:
            print("AP change iou 0.0")
            ap_change_miou = 0.0
        if length_pred50 != 0 and length_gt50 != 0:
            for c_gt_0, c_pred_0, c_gt_1, c_pred_1 in zip(change_gt_dict_0["all_ap_50%"], change_pred_dict_0["all_ap_50%"], change_gt_dict_1["all_ap_50%"], change_pred_dict_1["all_ap_50%"]):
                self.ap50iou_confusion_matrix.count_predicted_batch_list(c_gt_0, c_pred_0, c_gt_1, c_pred_1)
            ap50_change_miou = 100 * self.ap50iou_confusion_matrix.get_average_intersection_union()
        else:
            print("AP50 change iou 0.0")
            ap50_change_miou = 0.0
        if length_pred25 != 0 and length_gt25 != 0:
            for c_gt_0, c_pred_0, c_gt_1, c_pred_1 in zip(change_gt_dict_0["all_ap_25%"], change_pred_dict_0["all_ap_25%"], change_gt_dict_1["all_ap_25%"], change_pred_dict_1["all_ap_25%"]):
                self.ap25iou_confusion_matrix.count_predicted_batch_list(c_gt_0, c_pred_0, c_gt_1, c_pred_1)
            ap25_change_miou = 100 * self.ap25iou_confusion_matrix.get_average_intersection_union()
        else:
            print("AP25 change 0.0")
            ap25_change_miou = 0.0
        return ap_change_miou, ap50_change_miou, ap25_change_miou
    # def get_inst_metric(self, change_pred_dict_0, change_pred_dict_1, change_gt_dict_0, change_gt_dict_1):
    #     length_pred = len(change_pred_dict_0["all_ap"]) + len(change_pred_dict_1["all_ap"])
    #     length_gt = len(change_gt_dict_0["all_ap"]) + len(change_gt_dict_1["all_ap"])
    #     length_pred50 = len(change_pred_dict_0["all_ap_50%"]) + len(change_pred_dict_1["all_ap_50%"])
    ...
    #     if length_pred != 0 and length_gt != 0:
    #
    #         for c_gt_0, c_pred_0, c_gt_1, c_pred_1 in zip()
    #
    #         self.apiou_confusion_matrix.count_predicted_batch_list(change_gt_dict_0["all_ap"], change_pred_dict_0["all_ap"], change_gt_dict_1["all_ap"], change_pred_dict_1["all_ap"])
    #         ap_change_miou = 100 * self.apiou_confusion_matrix.get_average_intersection_union()
    #     else:
    #         print("AP change iou 0.0")
    #         ap_change_miou = 0.0
    #     if length_pred50 != 0 and length_gt50 != 0:
    #         self.ap50iou_confusion_matrix.count_predicted_batch_list(change_gt_dict_0["all_ap_50%"], change_pred_dict_0["all_ap_50%"], change_gt_dict_1["all_ap_50%"], change_pred_dict_1["all_ap_50%"])
    #         ap50_change_miou = 100 * self.ap50iou_confusion_matrix.get_average_intersection_union()
    #     else:
    #         print("AP50 change iou 0.0")
    #         ap50_change_miou = 0.0
    #     if length_pred25 != 0 and length_gt25 != 0:
    #         self.ap25iou_confusion_matrix.count_predicted_batch_list(change_gt_dict_0["all_ap_25%"], change_pred_dict_0["all_ap_25%"], change_gt_dict_1["all_ap_25%"], change_pred_dict_1["all_ap_25%"])
    #         ap25_change_miou = 100 * self.ap25iou_confusion_matrix.get_average_intersection_union()
    #     else:
    #         print("AP25 change 0.0")
    #         ap25_change_miou = 0.0
    ...
    def inst_metrics(self, dataset: str = "hkcd"):
        global CLASS_LABELS
        ...
        matches = {}
        # for i in np.arange(len(self.inst_preds)):
        #     pred = self.inst_preds.pop()
            # if self._stage == "test":
                # pos = pred["pos"]
                # full_pos = pred["full_pos"]
                # pred["pred_masks"] = pred["pred_masks"].numpy()
                # pred["pred_changes"] = np.squeeze(knn_interpolate(torch.unsqueeze(pred["pred_changes"], 1), pos, full_pos, k=1).numpy())
                # pred["pred_changes"] = pred["pred_changes"].astype(int)
                # knn_idx = pred["knn_idx"]
                # pred["pred_masks"] = pred["pred_masks"][knn_idx, :]
                # pred["pred_changes"] = pred["pred_changes"][knn_idx,]
                # pred["pred_changes"] = pred["pred_changes"].astype(int)
        print("assign_instances_for_scan")
        ...
        print("evaluate_matches")
        ap_scores, match_c_pred0, match_c_gt0, match_c_pred1, match_c_gt1 = evaluate_matches(matches)
        print("compute_averages")
        avgs, change_pred_dict_0, change_gt_dict_0, change_pred_dict_1, change_gt_dict_1 = compute_averages(ap_scores, match_c_pred0, match_c_gt0, match_c_pred1, match_c_gt1)
        print("get_inst_metric")
        ap_change_miou, ap50_change_miou, ap25_change_miou = self.get_inst_metric(change_pred_dict_0, change_pred_dict_1, change_gt_dict_0, change_gt_dict_1)

        return avgs['all_ap'], avgs['all_ap_50%'], avgs['all_ap_25%'], ap_change_miou, ap50_change_miou, ap25_change_miou
    ...
    def finalise(self, save_pc=False, name_test="", saving_path=None, conv_classes=None, num_class_cm=None, **kwargs):
        ...
        if self._stage != "train":
            ap, ap50, ap25, ap_change_miou, ap50_change_miou, ap25_change_miou = self.inst_metrics()
            self._mAP = ap
            self._mAP50 = ap50
            self._mAP25 = ap25
            self._mAP_cmiou = ap_change_miou
            self._mAP25_cmiou = ap50_change_miou
            self._mAP50_cmiou = ap25_change_miou

            print("ap {} ap50 {} ap25 {} ap_ciou {} ap50_ciou {} ap25_ciou {}".format(ap, ap50, ap25, ap_change_miou, ap50_change_miou, ap25_change_miou))

            # change_miou = self.change_confusion_matrix.get_average_intersection_union()
            # print("ap {} ap50 {} ap25 {} ap_ciou {} ap50_ciou {} ap25_ciou {} c_mIoU {}".format(ap, ap50, ap25, ap_change_miou, ap50_change_miou, ap25_change_miou, change_miou))
        ...
        if self.full_pc:
            ...
            for i, area in enumerate(self._areas):
                if area is not None:
                    ...
                    if self.full_res:
                        area_pos, area_orig_pos, _, _, _, _, _, _, gt0, gt1 = self._ds.clouds_loader(i, "vertex")
