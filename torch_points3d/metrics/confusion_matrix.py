...
class ConfusionMatrix:
    ...
    def count_predicted_batch_list(self, ground_truth_vec0, predicted0, ground_truth_vec1, predicted1):
        assert np.max(predicted0) < self.number_of_labels
        assert np.max(predicted1) < self.number_of_labels

        if torch.is_tensor(ground_truth_vec0):
            ground_truth_vec0 = ground_truth_vec0.numpy()
        if torch.is_tensor(predicted0):
            predicted0 = predicted0.numpy()

        if torch.is_tensor(ground_truth_vec1):
            ground_truth_vec1 = ground_truth_vec1.numpy()
        if torch.is_tensor(predicted1):
            predicted1 = predicted1.numpy()

        pred_valid = np.hstack((predicted0, predicted1 * 2))
        labels_valid = np.hstack((ground_truth_vec0, ground_truth_vec1 * 2))

        batch_confusion = np.bincount(
            self.number_of_labels * labels_valid.astype(int) + pred_valid, minlength=self.number_of_labels ** 2
        ).reshape(self.number_of_labels, self.number_of_labels)
        if self.confusion_matrix is None:
            self.confusion_matrix = batch_confusion
        else:
    ...
    def get_recall(self):
        ...
        return np.sum(Recall) / self.number_of_labels
