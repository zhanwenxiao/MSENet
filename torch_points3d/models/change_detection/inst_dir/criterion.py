...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_masks, mask_type):
        ...
        assert "pred_logits" in outputs
        src_logits = outputs["pred_logits"].float()
        new_taget, new_src_logits  = [], []
        for target, src_logit in zip(targets, src_logits):
            if "labels" not in target:
                continue
            new_taget.append(target)
            new_src_logits.append(src_logit)

        if len(new_src_logits) == 0 or len(new_taget) == 0:
            return {"loss_ce": torch.sum(torch.zeros([1,], device=outputs["pred_logits"].device))}

        new_src_logits = torch.stack(new_src_logits)
        idx = self._get_src_permutation_idx(indices)
        target_classes_o = torch.cat(
            [t["labels"][J] for t, (_, J) in zip(new_taget, indices)]
        )
        target_classes = torch.full(
            new_src_logits.shape[:2],
            self.num_classes,
            dtype=torch.int64,
            device=new_src_logits.device,
        )
        target_classes[idx] = target_classes_o

        loss_ce = F.cross_entropy(
            new_src_logits.transpose(1, 2),
            target_classes,
            self.empty_weight,
            ignore_index=253,
        )
        # if len(new_taget) == 0:
        #     return {"loss_ce": torch.zeros([1,], device=outputs["pred_logits"].device)}
        losses = {"loss_ce": loss_ce.to(outputs["pred_logits"].device)}
        return losses
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, mask_type):
        ...
        for batch_id, _ in enumerate(targets):
            if "labels" not in targets[batch_id]:
                continue
            map_id, target_id = indices[id]
        ...
        if len(loss_masks) == 0:
            return {
                "loss_mask": torch.sum(torch.zeros([1,], device=outputs["pred_logits"].device)),
                "loss_dice": torch.sum(torch.zeros([1,], device=outputs["pred_logits"].device)),
            }
        return {
