...
class BaseModel(torch.nn.Module, TrackerInterface, DatasetInterface, CheckpointInterface):
    ...
    def optimize_parameters(self, epoch, batch_size, loss_weight):
        """Calculate losses, gradients, and update network weights; called in every training iteration"""

        with torch.cuda.amp.autocast(enabled=self.is_mixed_precision()):  # enable autocasting if supported
            self.forward(epoch=epoch, loss_weight=loss_weight)  # first call forward to calculate intermediate results
    ...
    def set_grad_scale(self, _grad_scale):
        self._grad_scale = _grad_scale
