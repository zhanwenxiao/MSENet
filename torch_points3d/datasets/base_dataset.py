...
class BaseDataset:
    ...
    def has_inst_labels(self, stage: str) -> bool:
        """ Tests if a given dataset has labels or not
        ...
        sample = dataset[0]
        if hasattr(sample, "change_y_target") and hasattr(sample, "inst_y") and hasattr(sample, "sem_y"):
            return sample.change_y_target is not None and sample.inst_y is not None and sample.sem_y is not None
        return False
    ...
    @property
    def change_weight_classes(self):
        return getattr(self.train_dataset, "change_weight_classes", None)
    @property
    def inst_weight_classes(self):
        return getattr(self.train_dataset, "inst_weight_classes", None)
