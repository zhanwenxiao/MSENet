...
class Urb3DSimul(Dataset):
    ...
    def hand_craft_process(self,comp_normal=False):
        ...
        if not self.reload_preproc or not exist_file:
            for idx in range(len(self.filesPC0)):
                pc0, pc1, sem_label0, sem_label1, inst_label0, inst_label1, change_label0, change_label1 = self.clouds_loader(idx, nameInPly=self.nameInPly)
                pc0 = Data(pos=pc0, sem_y=sem_label0, inst_y=inst_label0, change_y=change_label0)
                pc1 = Data(pos=pc1, sem_y=sem_label1, inst_y=inst_label1, change_y=change_label1)
                if comp_normal:
    ...
    def get(self, idx):
        if self.pre_transform is not None:
            pc0, pc1, sem_label0, sem_label1, inst_label0, inst_label1, change_label0, change_label1 = self._preproc_clouds_loader(idx)
        else:
            pc0, pc1, sem_label0, sem_label1, inst_label0, inst_label1, change_label0, change_label1 = self.clouds_loader(idx, nameInPly=self.nameInPly)
    ...
    def clouds_loader(self, area, nameInPly = "params"):
        ...
        inst_gt0 = pc[:, 4].long()
        change_gt0 = pc[:, 5].long()

        return pc0.type(torch.float), pc1.type(torch.float), sem_gt0, sem_gt1, inst_gt0, inst_gt1, change_gt0, change_gt1
    ...
    def _preproc_clouds_loader(self, area):
        ...
        return data_pc0.pos, data_pc1.pos, data_pc0.sem_y, data_pc1.sem_y, data_pc0.inst_y, data_pc1.inst_y, data_pc0.change_y, data_pc1.change_y
...
class Urb3DSimulSphere(Urb3DSimul):
    ...
    def _load_save(self, i):
        if self.pre_transform is not None:
            pc0, pc1, sem_label0, sem_label1, inst_label0, inst_label1, change_label0, change_label1 = self._preproc_clouds_loader(i)
        else:
            pc0, pc1, sem_label0, sem_label1, inst_label0, inst_label1, change_label0, change_label1 = self.clouds_loader(i, nameInPly=self.nameInPly)
        pair = Pair(pos=pc0, pos_target=pc1, sem_y=sem_label0, sem_y_target=sem_label1, inst_y=inst_label0, inst_y_target=inst_label1, change_y=change_label0, change_y_target=change_label1)
...
class Urb3DSimulCylinder(Urb3DSimulSphere):
        ...
        if self._sample_per_epoch > 0:
            if self.fix_cyl:
                ...
                while not pair_correct and idx < self._centres_for_sampling_fixed.shape[0]:
                    ...
                    dataPC0 = Data(pos=pair.pos, idx=torch.arange(pair.pos.shape[0]).reshape(-1), sem_y=pair.sem_y, inst_y=pair.inst_y, change_y=pair.change_y)
                    setattr(dataPC0, CylinderSampling.KDTREE_KEY, pair.KDTREE_KEY_PC0)
                    dataPC1 = Data(pos=pair.pos_target, idx=torch.arange(pair.pos_target.shape[0]).reshape(-1), sem_y=pair.sem_y_target, inst_y=pair.inst_y_target, change_y=pair.change_y_target)
    ...
    def _get_random(self):
        ...
        dataPC0 = Data(pos=pair.pos, sem_y=pair.sem_y, inst_y=pair.inst_y, change_y=pair.change_y)
        setattr(dataPC0, CylinderSampling.KDTREE_KEY, pair.KDTREE_KEY_PC0)
        dataPC1 = Data(pos=pair.pos_target, sem_y=pair.sem_y_target, inst_y=pair.inst_y_target, change_y=pair.change_y_target)
        setattr(dataPC1, CylinderSampling.KDTREE_KEY, pair.KDTREE_KEY_PC1)
        dataPC0_cyl = cylinder_sampler(dataPC0)
        dataPC1_cyl = cylinder_sampler(dataPC1)
        if self.manual_transform is not None:
        ...
        pair_cyl = Pair(pos=dataPC0_cyl.pos, pos_target=dataPC1_cyl.pos, sem_y=dataPC0_cyl.sem_y, sem_y_target=dataPC1_cyl.sem_y,
                        inst_y=dataPC0_cyl.inst_y, inst_y_target=dataPC1_cyl.inst_y, change_y=dataPC0_cyl.change_y, change_y_target=dataPC1_cyl.change_y, area=area_sel)
        if self.DA:
